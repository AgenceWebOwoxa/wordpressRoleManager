...
class OWOXA_Role_Manager {
    ...
    public function __construct() {
        ...
        add_action( 'user_profile_update_errors', [ $this, 'protect_multiple_roles' ], 10, 3 );
    ...
    /**
     * Masque le select natif + affiche un message
     */
    ...
    public function hide_role_dropdown_css() {
        ...
        ?>
        <style>
            /* Cache le select Rôle natif de WordPress */
            .user-role-wrap {
    ...
    /**
     * Empêche WordPress d’écraser les multi-rôles à la sauvegarde du profil
     */
    public function protect_multiple_roles( $errors, $update, $user ) {
        if ( ! $update || empty( $user->ID ) ) {
            return;
        }

        $current_user = new WP_User( $user->ID );
        $roles_count  = count( (array) $current_user->roles );

        // Si l’utilisateur a plusieurs rôles, on empêche le set_role natif
        if ( $roles_count > 1 && isset( $_POST['role'] ) ) {
            // On supprime le champ role du POST pour que WordPress ne fasse pas de set_role
            unset( $_POST['role'] );
        }
    }
