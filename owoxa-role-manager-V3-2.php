 ...
 * Description: Gestion des rôles utilisateurs + édition des capabilities + capabilities personnalisées + multi-rôles + protection multi-rôles.
 * Version: 1.3.2
...
class OWOXA_Role_Manager {
    ...
    public function __construct() {
        ...
        add_action( 'show_user_profile', [ $this, 'hide_native_role_dropdown' ], 1 );
        add_action( 'edit_user_profile', [ $this, 'hide_native_role_dropdown' ], 1 );
        add_action( 'admin_head', [ $this, 'hide_role_dropdown_css' ] );

        // Sauvegarde et restauration des rôles multiples (méthode fiable)
        add_action( 'personal_options_update', [ $this, 'save_previous_roles' ] );
        add_action( 'edit_user_profile_update', [ $this, 'save_previous_roles' ] );
        add_action( 'profile_update', [ $this, 'restore_multiple_roles' ], 20, 2 );
    }
    ...
    /**
     * Récupère toutes les capabilities connues
     */
    ...
    /**
     * Masque le select natif + affiche un message si multi-rôles
     */
    public function hide_native_role_dropdown( $user ) {
        if ( ! current_user_can( 'promote_users' ) ) {
            return;
        }

        $roles_count = count( (array) $user->roles );

        if ( $roles_count > 1 ) {
            echo '<div class="notice notice-warning inline" style="margin: 15px 0;">';
            echo '<p><strong>Multi-Rôles actifs</strong> — Cet utilisateur a actuellement <strong>' . $roles_count . ' rôles</strong>.</p>';
            echo '<p>Le menu déroulant « Rôle » de WordPress a été désactivé pour éviter d’écraser les rôles multiples.</p>';
            echo '<p><a href="' . esc_url( admin_url( 'users.php?page=owoxa-multi-roles&user_id=' . $user->ID ) ) . '" class="button button-primary">Gérer les rôles multiples</a></p>';
            echo '</div>';
        }
    }

    /**
     * Cache le select natif via CSS
     */
    public function hide_role_dropdown_css() {
    ...
    /**
     * Sauvegarde temporairement les rôles avant la mise à jour du profil
     */
    public function save_previous_roles( $user_id ) {
        $user = new WP_User( $user_id );
        if ( count( (array) $user->roles ) > 1 ) {
            update_user_meta( $user_id, '_owoxa_previous_roles', $user->roles );
        }
    }

    /**
     * Restaure les rôles multiples après la mise à jour du profil
     */
    public function restore_multiple_roles( $user_id, $old_user_data ) {
        $previous_roles = get_user_meta( $user_id, '_owoxa_previous_roles', true );
        ...
        // On retire tout ce que WordPress a mis
        foreach ( $user->roles as $role ) {
            $user->remove_role( $role );
        }

        // On remet les rôles précédents
        foreach ( $previous_roles as $role ) {
            $user->add_role( $role );
        }

        // Nettoyage
        delete_user_meta( $user_id, '_owoxa_previous_roles' );
    }
            ...
            <?php if ( $user && $user->exists() ) : ?>
                ...
                <form method="post">
                    ...
                    <p class="description" style="margin-top:15px;">
                        <strong>Attention :</strong> l’utilisateur doit conserver au moins un rôle.<br>
